...
with col1:
    ...
    if st.session_state.deposit_type == "base_metals":
        metal_price = st.number_input("Prix du métal ($/kg)", min_value=0.0, value=8.0, step=0.1)
        cutoff_grade = st.slider("Teneur de coupure (%)", min_value=0.0, max_value=5.0, value=0.5, step=0.01)
        grade_unit = "%"
    else:  # Métaux précieux
        metal_price = st.number_input("Prix du métal ($/g)", min_value=0.0, value=60.0, step=1.0)
        ...
        if st.session_state.results_ready:
            ...
            with tab1:
                ...
                if st.session_state.deposit_type == "base_metals":
                    ...
                    # Pour les métaux de base (en pourcentage), conversion en kg pour le calcul du métal contenu
                ...
                if st.session_state.deposit_type == "base_metals":
                    total_revenue = recovered_metal * 1000 * metal_price  # prix par kg, métal en tonnes
                else:
                    total_revenue = recovered_metal * 1000 * metal_price  # prix par g, métal en kg
    ...
    for block in st.session_state.block_model:
        ...
        if st.session_state.deposit_type == "base_metals":
            ...
            if block['grade'] > cutoff_grade_decimal:
                ...
                metal_kg = tonnage * (block['grade'] / 100)  # kg de métal
                revenue = metal_kg * metal_price * recovery_decimal
            ...
            if block['grade'] > cutoff_grade_decimal:
                ...
                metal_g = tonnage * block['grade']  # grammes de métal
                revenue = metal_g * metal_price * recovery_decimal
                block['value'] = revenue - tonnage * (mining_cost + processing_cost)
            else:
                # Bloc de stérile
                block['value'] = -tonnage * mining_cost
